...
function on_thread_submit(button) {
	...
	if(name.length > 50 || name.split(/\r\n|\r|\n/).length > 1 ) {
		start_thread_status_set('Error : Name too long', 'err_A')
		return
	}
	if(subject.length > 100 || subject.split(/\r\n|\r|\n/).length > 1 ) {
		start_thread_status_set('Error : Subject too long', 'err_A')
		return
	}

	if(text.length > 1500 || text.split(/\r\n|\r|\n/).length > 40 ) {
		start_thread_status_set('Error : Text too long', 'err_A');
		return
	}

	if (text.length === 0 && subject.length === 0 ) {
		start_thread_status_set('Error : Empty content', 'err_A');
		return
	}

	if( image == null ) {
		start_thread_status_set('Error : No image attached', 'err_A')
		return
	}
	...
	function on_fail(status, d) {
		if(status == 500 || status == 400) {
			start_thread_status_set('Error : ' + d, 'err_B')
		} 
		else if(status == 413) {
			start_thread_status_set('Error : image size too large', 'err_B')
		}
		else {
			start_thread_status_set('Error : ' + status, 'err_B')			
		}
